...
Muljax release on GitHub, verifies its integrity, extracts it, and installs it.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := update.DownloadFile(nil, archiveDownloadURL, archivePath); err != nil {
				return fmt.Errorf("failed to download release archive: %w", err)
			}

			if checksumsURL != "" {
				checksumsPath := filepath.Join(tmpDir, "checksums.txt")
				if err := update.DownloadFile(nil, checksumsURL, checksumsPath); err == nil {
					ui.StepInfo("Verifying archive integrity...")
					if err := update.VerifyChecksum(archivePath, checksumsPath, archiveName); err != nil {
			...
			extractedBinPath, err := update.ExtractArchive(archivePath, tmpDir, goos)
			if err != nil {
